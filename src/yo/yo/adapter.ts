...
export default class CodeAdapter {
	...
	public prompt(questions:any, callback:any) {
		...
		const promise = questions.reduce((promise:any, question:any) => {
			return promise
				.then(() => {
				...
				.then((askQuestion:any) => {
					if (askQuestion) {
						const prompt = PromptFactory.createPrompt(question, answers);

						return prompt.render().then((result:any) => answers[question.name] = question.filter ? question.filter(result) : result);
					}
				});
		}, Promise.resolve());
	...
	public diff(actual:any, expected:any) {
		this.outChannel.clear();
